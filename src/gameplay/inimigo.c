...
void inimigo_atualizar(Inimigo* inimigo, const Mapa* mapa, float delta_tempo)
{
    ...
    // permitindo que o inimigo avançasse quase um tile a mais antes de inverter.
    ...
    // Parede: limite do mapa ou tile Z à frente no mesmo nível
    ...
    // Borda: sem plataforma abaixo na coluna de borda (escadas também contam como chão)
    borda = false;
...
bool inimigo_colide_com_jogador(const Inimigo* inimigo, const Jogador* jogador)
{
    ...
    // Bounding boxes em pixels; cada entidade ocupa exatamente TILE_SIZE × TILE_SIZE
